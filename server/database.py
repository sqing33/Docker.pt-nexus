...
class DatabaseManager:
    ...
    def init_db(self):
        ...
        # 表创建逻辑 (PostgreSQL)
            ...
            # 创建种子参数表，用于存储从源站点提取的种子参数
            cursor.execute(
                "CREATE TABLE IF NOT EXISTS seed_parameters (hash VARCHAR(40) NOT NULL, torrent_id VARCHAR(255) NOT NULL, site_name VARCHAR(255) NOT NULL, nickname VARCHAR(255), save_path TEXT, name TEXT, title TEXT, subtitle TEXT, imdb_link TEXT, douban_link TEXT, type VARCHAR(100), medium VARCHAR(100), video_codec VARCHAR(100), audio_codec VARCHAR(100), resolution VARCHAR(100), team VARCHAR(100), source VARCHAR(100), tags TEXT, poster TEXT, screenshots TEXT, statement TEXT, body TEXT, mediainfo TEXT, title_components TEXT, removed_ardtudeclarations TEXT, downloader_id VARCHAR(36), is_deleted BOOLEAN NOT NULL DEFAULT FALSE, is_reviewed BOOLEAN NOT NULL DEFAULT FALSE, mediainfo_status VARCHAR(20) DEFAULT 'pending', bdinfo_task_id VARCHAR(36), bdinfo_started_at TIMESTAMP, bdinfo_completed_at TIMESTAMP, bdinfo_error TEXT, created_at TIMESTAMP NOT NULL, updated_at TIMESTAMP NOT NULL, PRIMARY KEY (hash, torrent_id, site_name))"
            )
            # 创建批量转种记录表
            cursor.execute(
