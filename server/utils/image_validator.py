...
def is_image_url_valid_robust(url: str) -> bool:
    ...
    headers = {
        'User-Agent':
        'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/142.0.0.0 Safari/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36',
    ...
    if any(domain in url.lower()
           for domain in ['pixhost.to', 'img.seedvault.cn', 'ptimg.org']):
    ...
    try:
        ...
        if any(ct in content_type for ct in valid_content_types):
            ...
            logging.warning(
                f"链接有效但Content-Type异常: {url} (Content-Type: {content_type})")
            # 尝试内容验证
            return _verify_image_content(url, headers)
        ...
        try:
            ...
            try:
                ...
                return _verify_image_content(url,
                                             fallback_headers,
                                             stream=True)
...
def _verify_image_content(url: str,
                          headers: dict,
    ...
    try:
        ...
        if content.startswith(
                b'ftyp') and b'heic' in content or b'heif' in content:
