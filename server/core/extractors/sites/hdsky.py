...
class HDSkySpecialExtractor:
    """HDSky 站点的特殊提取器 - 仅用于特殊的 tag 提取，其他信息使用公共提取器"""

    def __init__(self, soup: BeautifulSoup, base_url: str, cookie: str, torrent_id: str):
        """
        初始化 HDSky 特殊提取器
    ...
    def extract_tags(self, main_title: str):
        ...
        Returns:
            标签列表或 False（如果未找到种子）
        """
        tags = TorrentListFetcher.get_tags(
            self.base_url, 
            self.cookie,
            main_title, 
            self.torrent_id
        ...
        if tags is False:
            error_msg = (
                f"在站点搜索结果中未找到 ID 为 {self.torrent_id} 的种子！"
    ...
    def extract_all(self, torrent_id=None):
        ...
        # [优化] 步骤1: 先只提取标题（最小开销）
        print("[HDSky] 步骤1: 提取标题...")
        h1_top = self.soup.select_one("h1#top")
        if not h1_top:
            error_msg = "未找到标题元素"
            print(f"[HDSky] 错误: {error_msg}")
            return {
                "error": True,
                "message": error_msg
            }
        
        main_title = list(h1_top.stripped_strings)[0] if h1_top.stripped_strings else ""
        if not main_title:
            error_msg = "标题为空"
            print(f"[HDSky] 错误: {error_msg}")
            return {
        ...
        # 标准化标题
        import re
        main_title = re.sub(r'(?<!\d)(?<!H)(?<!x)\.|\.(?!\d\b)(?!264)(?!265)', ' ', main_title)
        main_title = re.sub(r'\s+', ' ', main_title).strip()
        print(f"[HDSky] 成功提取标题: {main_title}")
        
        # [优化] 步骤2: 立即提取标签，如果失败则停止
        print("[HDSky] 步骤2: 提取标签...")
        try:
            ...
            print(f"[HDSky] 成功提取标签: {filtered_tags}")
            
        except ValueError as e:
            # 如果标签提取失败，立即返回错误，不再继续提取其他信息
            print(f"[HDSky] 标签提取失败，停止处理: {e}")
            return {
                "error": True,
                "message": str(e)
            }
        ...
        print("[HDSky] 步骤3: 标签验证成功，开始提取其他信息...")
        public_extractor = Extractor()
        extracted_data = public_extractor._extract_with_public_extractor(self.soup)
